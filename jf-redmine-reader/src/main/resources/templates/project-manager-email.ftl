...
<h1>Validated documents</h1>

<p>These justification documents are now validated and await to be locked.</p>

<ul>
    <#list validated as validatedDocument>
        <li><a href="${redmineUrl}/projects/${projectName}/wiki/${validatedDocument.associatedPage.title}">${validatedDocument.associatedPage.title}</a></li>
    </#list>
</ul>
</#if>

<#if warnings?has_content || errors?has_content || others?has_content>
<h1>Your issues</h1>
